<script>
    ...
    async function loadData(){
      ...
      loading = true;
      let data = await fetch(`https://events.startupmission.in/api/event/beyondtomorrow_2026/agenda/venue`).then(response => response.json());
              ...
              {#each agenda as {name, description, start_time, end_time, category, speakers} }
                ...
                <div class=" alternate-row px-5 py-3">
                  <div class="flex flex-col md:flex-row md:space-x-5 ">
                    <div class=" text-[#0f0834]  flex-shrink-0 min-w-[6rem]"><p class=" text-[#0f0834] text-[15px] items-center ">{date_single(start_time)} IST</p></div>
                    
                    <div class="w-full divide-y divide-gray-600 space-y-4">
                      
                        <div class=" w-full ">
                          <div class=" uppercase "><p class=" text-[#591CBC] text-[13px] items-center ">{category ?? ""}</p></div>
                          <div class="text-[15px]  font-bold max-w-xl "><p class="text-[#0f0834] font-semibold">{name}</p></div>
      
                          {#if description }
                            <div class="mt-4 text-[#0f0834] mb-4">
                          ...
                          {#if Object.entries(speakers).length}
                            <div class="mt-4 ml-4">
                              {#each Object.entries(speakers) as [category, speaker_list]}
                                <div class="font-bold uppercase  text-black mb-3">
                                ...
                                <div class="flex flex-wrap w-full">
                                  {#each speaker_list as {name, designation, organisation, photo, linkedin}}
                                    <div class="flex items-center space-x-4 mb-3 pr-3 md:w-1/2">
                                      <div class="rounded-full flex-shrink-0 w-20 border-r-2 border-b-2 border-blue-200 shadow-lg overflow-hidden">
                                        <img src="{photo}" alt="name" class="">
                                      </div>
                                      <div>
                                        <div class="font-bold text-[#0f0834]">{name}</div>
                                        <div class=" text-[12px]  text-[#FF5A1F]">{designation}</div>
                                        <div class="text-[12px]  text-[#FF5A1F]">{organisation ?? ""}</div>
                                      </div>
                                    </div>
                                  {/each}
                                </div>
                          ...
                          <!-- {#if venue}
                            <div class="pt-3 text-xs  flex space-x-4 text-[#0f0834]">
  ...
  <style lang="scss">
    ...
    #event-sched{
      position: relative;
      &::before{
        @apply text-[#0f0834] md:text-8xl text-5xl font-bold  opacity-10 transform -rotate-90;
