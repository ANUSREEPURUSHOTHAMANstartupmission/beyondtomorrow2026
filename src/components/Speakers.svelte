<script>
    ...
    onMount(()=>{
      fetch(`https://events.startupmission.in/api/event/beyond_tomorrow_2025/speakers?category=speaker`)
  ...
  {#each Object.entries(speaker_list) as [category, speakers]}
    <div class="mx-auto speakers__container overflow-hidden { full_view ? "" : "max-h-[1000px]"} openTrans relative">
      ...
      <div class="grid grid-cols-1 gap-3  lg:grid-cols-4 md:gap-10 md:grid-cols-3 sm:gap-5">
  
     
  
        {#each speakers as {name, designation, organisation, photo, linkedin}}
  
          <div class="max-w-[19rem] speaker items-center justify-center">
        
            <div class="bg-cover rounded-2xl h-[18rem] items-center justify-center scale-100  w-full mx-auto  cursor-pointer bg-center" style={ 'background-image: url(' +photo+');' }>
            ...
            <div class="content">
              ...
              <p class="text-white text-base">{designation}</p>
              {#if organisation}
                <p class="text-white text-base">{organisation}</p>
              {/if}
              <div class="mt-2 h-1 w-20 bg-[#F15A29]"></div>
            </div>
